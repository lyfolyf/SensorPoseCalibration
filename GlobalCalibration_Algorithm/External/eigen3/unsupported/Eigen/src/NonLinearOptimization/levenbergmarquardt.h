...
struct DisParameter //add by xia
{
	double initDis;
	double reduceRatio;
	double currentDis;
	DisParameter() :initDis(0), reduceRatio(0), currentDis(0) {}
	DisParameter(double _init,double _ratio,double _curr):initDis(_init),reduceRatio(_ratio), currentDis(_curr)
	{}
};
...
public:
	...
	LevenbergMarquardtSpace::Status BestFitMinimize(FVectorType  &x, DisParameter& disPar,int maxIterNum, int& iterNum);
...
template<typename FunctorType, typename Scalar>
LevenbergMarquardtSpace::Status
LevenbergMarquardt<FunctorType, Scalar>::BestFitMinimize(FVectorType  &x,DisParameter& disPar,int maxIterNum,int& iterNum)
{
	LevenbergMarquardtSpace::Status status = minimizeInit(x);
	if (status == LevenbergMarquardtSpace::ImproperInputParameters)
		return status;
	iterNum = 0;
	do{
		if (iterNum >= maxIterNum) return LevenbergMarquardtSpace::Running;
		iterNum++;
		status = minimizeOneStep(x);
		disPar.currentDis *= disPar.reduceRatio;
	} while (status == LevenbergMarquardtSpace::Running);
	return status;
}
